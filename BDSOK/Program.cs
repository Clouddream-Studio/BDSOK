...
Dictionary<string, List<string>> webDatas = JsonConvert.DeserializeObject<Dictionary<string, List<string>>>(data);
Directory.CreateDirectory("BDSOK_cache");
foreach (var webData in webDatas)
{
    try
    {
        foreach (string runArgs in webData.Value)
        {
            string[] runArg = runArgs.Split(' ');
            switch (runArg[0])
            {
                case "show":
                    Logger.Trace($"[{webData.Key}] {runArgs[5..]}", Logger.LogLevel.INFO);
                    break;
                case "downloadmatch":
                    Logger.Trace($"[{webData.Key}] 开始从{runArg[1]}获取下载链接");
                    ...
                    Logger.Trace($"[{webData.Key}] 开始从{link}下载至{runArg[3]}");
                    webClient.DownloadFile(link, runArg[3]);
                    break;
                case "download":
                    Logger.Trace($"[{webData.Key}] 开始从{runArg[1]}下载至{runArg[2]}");
                    webClient.DownloadFile(runArg[1], runArg[2]);
                    break;
                case "unzip":
                    Logger.Trace($"[{webData.Key}] 开始解压{runArg[1]}到{runArg[2]}");
                    new FastZip().ExtractZip(runArg[1], runArg[2], null);
                    break;
                case "copyfile":
                    Logger.Trace($"[{webData.Key}] 开始复制文件{runArg[1]}到{runArg[2]}");
                    File.Copy(runArg[1], runArg[2]);
                    break;
                case "copydir":
                    Logger.Trace($"[{webData.Key}] 开始复制文件夹{runArg[1]}内文件到{runArg[2]}");
                    CopyDir(runArg[1], runArg[2]);
                    break;
                case "writefile":
                    Logger.Trace($"[{webData.Key}] 开始覆写{runArg[1]}");
                    File.WriteAllText(runArg[1], runArgs[(11 + runArg[1].Length)..]);
                    break;
                case "appendfile":
                    Logger.Trace($"[{webData.Key}] 开始写入{runArg[1]}");
                    File.AppendAllText(runArg[1], runArgs[(12 + runArg[1].Length)..]);
                    break;
                case "mkdir":
                    Logger.Trace($"[{webData.Key}] 开始新建{runArg[1]}");
                    Directory.CreateDirectory(runArg[1]);
                    break;
                case "delfile":
                    Logger.Trace($"[{webData.Key}] 开始删除文件{runArg[1]}");
                    File.Delete(runArg[1]);
                    break;
                case "deldir":
                    Logger.Trace($"[{webData.Key}] 开始删除文件夹{runArg[1]}");
                    Directory.Delete(runArg[1]);
                    break;
                case "start":
                    Logger.Trace($"[{webData.Key}] 开始启动{runArg[1]}");
